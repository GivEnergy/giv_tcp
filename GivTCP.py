...
class GivTCP:
    ...
    def publish_to_MQTT(topic, payload):
        ...
        for reg in payload:
            ...
            client.publish(
                "GivEnergy/" + GivTCP.dataloggerSN + "/" + topic + "/" + reg,
                payload[reg],
            )
    ...
    def TCP_call(inputRegister, inputFunction, inputStep):
        ...
        crc = (
            CrcModbus()
            .process(
                bytearray.fromhex(inputFunctionHex + inputRegisterHex + inputStepHex)
            )
            .finalhex()
        )
        dataSize = GivTCP.int_to_hex_string(
            int(
                len(
                    DEVICE_ADDRESS
                    + DATALOGGER_FUNCTION_CODE
                    + serial_number
                    + FILLER
                    + inputFunctionHex
                    + inputRegisterHex
    ...
    def read_register(inputRegister, inputFunction, inputStep):
        ...
        while (
            data == "" and n < 3
        ):  # Try to get register data upto 3 times before giving up
        ...
        if data != "":
            ...
            for reg in registers:
                if inputFunction == "04":
                    ...
                    key = (
                        GivTCP.input_register_LUT.get(registerInt + j)[0]
                        + "("
                        + str(registerInt + j)
                        + ")"
                    )
                    ...
                    key = (
                        GivTCP.holding_register_LUT.get(registerInt + j)[0]
                        + "("
                        + str(registerInt + j)
                        + ")"
                    )
                ...
                j = j + 1
                if j >= stepInt:
                    break  # Handle cases where invertor send erroneous additional data
    ...
    def getPowerData():
        ...
        if len(PV_stats) == 3:
            PVPower = (
                PV_stats[GivTCP.input_register_LUT.get(18)[0] + "(18)"]
                + PV_stats[GivTCP.input_register_LUT.get(20)[0] + "(20)"]
            )
        ...
        if len(load_power) != 0:
            power_output["Load Power"] = load_power[
                GivTCP.input_register_LUT.get(42)[0] + "(42)"
            ]
        ...
        if len(eps_power) != 0:
            power_output["EPS Power"] = eps_power[
                GivTCP.input_register_LUT.get(31)[0] + "(31)"
            ]
        ...
        if (
            len(power_output) == 10
        ):  # Only publish if all values are there, otherwise values don't match up...
    ...
    def getCombinedStats():
        ...
        if len(temp_output) == 60:
            power_output["PV Power"] = (
                temp_output[GivTCP.input_register_LUT.get(18)[0] + "(18)"]
                + temp_output[GivTCP.input_register_LUT.get(20)[0] + "(20)"]
            )

            temphex = str(
                temp_output[GivTCP.input_register_LUT.get(21)[0] + "(21)"]
            ) + str(temp_output[GivTCP.input_register_LUT.get(22)[0] + "(22)"])
            kwh_value = round(
                int(temphex, 16) * GivTCP.input_register_LUT.get(21)[2], 2
            )
            energy_output["Export Energy Total kwh"] = kwh_value

            temphex = str(
                temp_output[GivTCP.input_register_LUT.get(27)[0] + "(27)"]
            ) + str(temp_output[GivTCP.input_register_LUT.get(28)[0] + "(28)"])
            kwh_value = round(
                int(temphex, 16) * GivTCP.input_register_LUT.get(27)[2], 2
            )
            energy_output["Load Energy Total kwh"] = kwh_value

            temphex = str(
                temp_output[GivTCP.input_register_LUT.get(32)[0] + "(32)"]
            ) + str(temp_output[GivTCP.input_register_LUT.get(33)[0] + "(33)"])
            kwh_value = round(
                int(temphex, 16) * GivTCP.input_register_LUT.get(32)[2], 2
            )
            ...
            power_output["EPS Power"] = temp_output[
                GivTCP.input_register_LUT.get(31)[0] + "(31)"
            ]

            power_output["Load Power"] = temp_output[
                GivTCP.input_register_LUT.get(42)[0] + "(42)"
            ]
            temphex = str(
                temp_output[GivTCP.input_register_LUT.get(45)[0] + "(45)"]
            ) + str(temp_output[GivTCP.input_register_LUT.get(46)[0] + "(46)"])
            kwh_value = round(
                int(temphex, 16) * GivTCP.input_register_LUT.get(45)[2], 2
            )
            ...
            power_output["SOC"] = temp_output[
                GivTCP.input_register_LUT.get(59)[0] + "(59)"
            ]
